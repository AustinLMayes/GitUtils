...
def to_master
  current  = Git.current_branch
  target = current.end_with?("mco") ? "master-mco-gameframework" : "master-gameframework"
  system "git", "checkout", target
  Git.ensure_branch target
  info "Merging #{current} into #{target}"
  return system "git", "merge", current, "--no-edit"
...
desc "Deploy the @urrent branch to master-gameframework, merge into MCO, and deploy that to master-mco-gameframework"
...
desc "Deploy the current branch to master-gameframework, cherry-pick into MCO, and deploy that to master-mco-gameframework"
...
def deploy(verb, &block)
  ...
  if to_gamedev
    ...
    if to_master
      info "Merged #{mco_name} to master-mco-gameframework"
      system "git", "checkout", base_name
      wait_range 5, 25 if $delays_enabled
      info "Pusing all branches to remote"
      push_all "master-gameframework", "master-mco-gameframework", base_name, mco_name
      info "Pushed all branches to remote"
    else
      error "Merge to master-mco-gameframework failed"
    end
  else
...
desc "Push master-gameframework,master-mco-gameframework,curbranch, and curbranch-mco"
task push_up: :before do |task, args|
  ...
  info "Pusing all branches to remote"
  push_all "gamedevnet","gamedevnet-mco","master-gameframework", "master-mco-gameframework", base_name, mco_name
end
...
def make_prs(title, multi, slack)
  ...
  if multi
    ...
    res << GitHub.make_pr(title, base: "production-mco-gameframework", suffix: "[GFW-MCO]")
  else
    base_branch = Git.current_branch.end_with?("mco") ? "production-mco-gameframework" : "production-gameframework"
    res << GitHub.make_pr(title, suffix: "", base: base_branch)
  end
...
task new_mco: :before do |task, args|
  make_branch args.extras[0] + "-mco", "production-mco-gameframework"
end
